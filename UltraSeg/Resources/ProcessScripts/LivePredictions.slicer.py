...
f = open(r"LivePredictionsLog.txt", "w")
...
def resizeInputArray(input_array):
  ...
  resized_input_array = np.array(
    input_image.resize(
      (
        512, #int(input_image.width * slicer_to_model_scaling[1]),  # M direction (width on US machine)
        512, #int(input_image.height * slicer_to_model_scaling[0]),  # F direction (height on US machine)
      ),
  ...
  resized_input_array = np.expand_dims(resized_input_array, axis=(0,1))  # Add Batch and channel dimension
  # resized_input_array = np.expand_dims(resized_input_array, axis=0)  # Add channels last dimension
  # f.write(str(resized_input_array.shape))
  return torch.as_tensor(resized_input_array).float().cuda()
...
def resizeOutputArray(y):
  ...
  upscaled_output_array = np.array(
    output_image.resize(
      (
        image_shape[0], #int(output_image.width * model_to_slicer_scaling[1]),
        image_shape[1], #int(output_image.height * model_to_slicer_scaling[0]),
      ),
      resample=Image.BILINEAR,
    )
  )
...
try:
  ...
  checkpoint = torch.load(input_data['model_path'])
  ...
  # )
  image_shape = input_data['volume'].shape[0], input_data['volume'].shape[1]
